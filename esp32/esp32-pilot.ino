...
// LED — XIAO ESP32S3 built-in LED (pin 21, active LOW)
#define LED_PIN 21
unsigned long lastLedToggle = 0;
bool ledOn = false;

void ledSolid()  { digitalWrite(LED_PIN, LOW); ledOn = true; }
void ledOff()    { digitalWrite(LED_PIN, HIGH); ledOn = false; }
...
void loop() {
  ...
  // LED: solid red when connected, flashing red when waiting
  if (deviceConnected) {
    if (!ledOn) ledSolid();
  } else {
